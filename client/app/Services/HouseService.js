...
class HouseService {
    ...
    async freeRealEstate() {
        await ProxyState.houses.forEach(h => api.put("houses/" + h.id, { price: 0 }))

    }
    async fillInDescrips() {
        await ProxyState.houses.forEach(h => {
            h.description ? '' : api.put("houses/" + h.id, { description: "default description string" })
            h.description ? '' : h.description = "default description string"
            ProxyState.houses = ProxyState.houses
        })
